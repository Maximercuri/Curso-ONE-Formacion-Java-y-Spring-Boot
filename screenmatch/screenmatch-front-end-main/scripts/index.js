    ...
    const listaHTML = datos.map((pelicula) => `
        <li>
            <a href="/detalles.html?id=${pelicula.id}">
                <img src="${pelicula.imagenPromocional}" alt="${pelicula.titulo}">
            </a>
        </li>
    `).join('');
...
const sectionsParaOcultar = document.querySelectorAll('.section'); // Adicione a clase CSS 'hide-when-filtered' las secciones e títulos que desea ocultar.
